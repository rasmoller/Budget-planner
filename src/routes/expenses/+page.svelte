...
{#if showModal}
	<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
		<div class="bg-[var(--color-surface)] rounded-lg p-6 w-full max-w-md mx-4">
			...
			<div>
				<label for="exp-amount" class="block text-sm font-medium mb-1">{formIsVariable ? $t.field.estimate : $t.field.amount}</label>
				<input
					id="exp-amount"
					type="number"
				...
				<div class="flex items-center gap-2">
					<input
						...
						class="w-4 h-4"
					/>
					<label for="exp-onetime" class="text-sm font-medium">{$t.entry.oneTime}</label>
				...
				{#if !formIsOneTime}
					<div class="flex items-center gap-2">
						<input
							...
							class="w-4 h-4"
						/>
						<label for="exp-variable" class="text-sm font-medium">{$t.entry.variable}</label>
						<span class="text-xs text-gray-500">{$t.entry.variableHint}</span>
					</div>
					{#if formIsVariable}
						<div class="grid grid-cols-2 gap-2">
							<div>
								<label for="exp-min" class="block text-xs font-medium mb-1">{$t.field.min}</label>
								<input
									id="exp-min"
									type="number"
									min="0"
							...
							<div>
								<label for="exp-max" class="block text-xs font-medium mb-1">{$t.field.max}</label>
								<input
									id="exp-max"
									type="number"
									min="0"
				...
				{#if formIsOneTime}
					<div>
						<label for="exp-date" class="block text-sm font-medium mb-1">{$t.field.date}</label>
						<input
							id="exp-date"
							type="date"
							bind:value={formDate}
